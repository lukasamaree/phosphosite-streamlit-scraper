...
async def run_agentic_lookup(protein_names, output_csv, state_json, config):
    ...
    async with async_playwright() as p:
        print("SESSION: launching one browser context for this lookup batch", flush=True)
                ...
                for attempt in range(1, config.attempts + 1):
                    try:
                        print(
                            ...
                            "using shared browser session",
                            flush=True,
                        )
                        result = await resolve_protein_name_on_page(page, protein_name, config.organism)
                        runs[protein_name] = {
                            **result,
                            "status": "resolved",
                            "attempts": attempt,
                        ...
                        last_error = str(exc)
                        error_type = "cloudflare" if is_cloudflare_error(exc) else "lookup"
                        runs[protein_name] = {
